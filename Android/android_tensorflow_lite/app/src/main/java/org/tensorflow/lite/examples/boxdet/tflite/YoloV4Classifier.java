...
package org.tensorflow.lite.examples.boxdet.tflite;
...
import org.tensorflow.lite.examples.boxdet.MainActivity;
...
public class YoloV4Classifier implements Classifier {
    ...
    public static Classifier create(
        ...
        //Log.e("Awei","numBytesPerChannel:" + numBytesPerChannel);
        // Sofia Change
        d.imgData = ByteBuffer.allocateDirect(BATCH_SIZE * d.INPUT_SIZE  * d.INPUT_SIZE  * 3 * numBytesPerChannel);
    ...
    // Float model
    private static final float IMAGE_MEAN = 128;//0;

    private static final float IMAGE_STD = 128.0f;//255.0f;
    ...
    // tiny or not
    private static boolean isTiny = true;//false;

    // config yolov4 tiny
    private static final int[] OUTPUT_WIDTH_TINY_256 = new int[]{960, 960};
    private static final int[] OUTPUT_WIDTH_TINY_320 = new int[]{1500, 1500};
    private static final int[] OUTPUT_WIDTH_TINY_416 = new int[]{2535, 2535};
    ...
    private static final int[] ANCHORS_TINY = new int[]{
            10,14,  23,27,  37,58,  81,82,  135,169,  344,319};//23, 27, 37, 58, 81, 82, 81, 82, 135, 169, 344, 319};
    ...
    protected static final int BATCH_SIZE = 1;//64;
...
//    }
    ...
    private ArrayList<Recognition> getDetectionsForTiny(ByteBuffer byteBuffer, Bitmap bitmap) {
        ...
        if (INPUT_SIZE == 256)
        {
            tmp_grid_w = OUTPUT_WIDTH_TINY_256[0];
            outputMap.put(0, new float[1][OUTPUT_WIDTH_TINY_256[0]][4]);
            outputMap.put(1, new float[1][OUTPUT_WIDTH_TINY_256[1]][labels.size()]);
        }
        else if (INPUT_SIZE == 320)
        {
            tmp_grid_w = OUTPUT_WIDTH_TINY_320[0];
            outputMap.put(0, new float[1][OUTPUT_WIDTH_TINY_320[0]][4]);
            outputMap.put(1, new float[1][OUTPUT_WIDTH_TINY_320[1]][labels.size()]);
        }
        else
        {
            tmp_grid_w = OUTPUT_WIDTH_TINY_416[0];
            outputMap.put(0, new float[1][OUTPUT_WIDTH_TINY_416[0]][4]);
            outputMap.put(1, new float[1][OUTPUT_WIDTH_TINY_416[1]][labels.size()]);
        }
        Object[] inputArray = {byteBuffer};
        tfLite.runForMultipleInputsOutputs(inputArray, outputMap);

        int gridWidth = tmp_grid_w;//OUTPUT_WIDTH_TINY[0];
        float[][][] bboxes = (float [][][]) outputMap.get(0);
        float[][][] out_score = (float[][][]) outputMap.get(1);
